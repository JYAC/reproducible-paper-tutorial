...
Before we go any further, save the file (via the menu, 'file', then 'save'). Just call it something like 'example'.
...
If you click on "Knit" the RMarkdown file will run and generate all results and present you with a HTML file, PDF file, or a Word file. If RStudio requests you to install packages, click yes and see whether everything works to begin with. 

A window should appear containing the report and you should be able to find the corresponding file in your folder. Try changing some things in the R markdown file then rekniting to see the changes in the final report. For example, change the title of the report from 'untitled' to 'example'.
...
Isn't this great?? Now when we write our research papers there is a unbroken chain directly from the numbers we report in the paper their source.
